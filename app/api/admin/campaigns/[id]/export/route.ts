...
export async function GET(
  ...
  assertExportAllowed(session, campaign.id);
...
`;
    ...
    return new NextResponse(txt, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${safeName}_${dateStr}.txt"`,
        "X-Total-Records": String(filtered.length),
      },
    });
  }
  ...
  return new NextResponse(jsonl, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${safeName}_${dateStr}.jsonl"`,
      "X-Total-Records": String(filtered.length),
    },
  });
}