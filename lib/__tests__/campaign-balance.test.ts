...
beforeEach(async () => {
  await truncateAll();
  process.env = { ...ORIGINAL_ENV, PLATFORM_FEE_STROOPS: "1500000" };
});

// ── getPlatformFeeStroops ──────────────────────────────────────────────────────────

describe("getPlatformFeeStroops", () => {
  it("parses env var as BigInt", () => {
    process.env.PLATFORM_FEE_STROOPS = "1500000";
    expect(getPlatformFeeStroops()).toBe(1500000n);
  });

  it("throws when env var is missing", () => {
    delete process.env.PLATFORM_FEE_STROOPS;
    expect(() => getPlatformFeeStroops()).toThrow("PLATFORM_FEE_STROOPS");
  });

  it("throws when env var is not a valid integer string", () => {
    process.env.PLATFORM_FEE_STROOPS = "not-a-number";
    expect(() => getPlatformFeeStroops()).toThrow("PLATFORM_FEE_STROOPS");
  });
});
...
describe("checkAndDebit", () => {
  it("debits labeler reward + platform fee from balance", async () => {
    const campaign = await createCampaign({ rewardStroops: 500000n });
    await createCampaignBalance(campaign.id, 10000000n);

    await checkAndDebit(campaign.id, 500000n, "sub-001");

    const balance = await prisma.campaignBalance.findUnique({ where: { campaignId: campaign.id } });
    // 1 XLM - 0.05 (reward) - 0.15 (fee) = 0.8 XLM = 8000000 stroops
    expect(balance?.balanceStroops).toBe(8000000n);
  });
  ...
  it("succeeds and leaves 0 balance when balance exactly equals required", async () => {
    ...
    const balance = await prisma.campaignBalance.findUnique({ where: { campaignId: campaign.id } });
    expect(balance?.balanceStroops).toBe(0n);
  });
});
...
describe("getBalanceSummary", () => {
  it("returns balanceStroops and estimated submissions remaining", async () => {
    const campaign = await createCampaign({ rewardStroops: 500000n });
    await createCampaignBalance(campaign.id, 4000000n);

    // PLATFORM_FEE_STROOPS=1500000, rewardStroops=500000
    ...
    expect(summary.balanceStroops).toBe(4000000n);
  ...
  it("returns 0 remaining when no balance row exists", async () => {
    ...
    expect(summary.balanceStroops).toBe(0n);
    expect(summary.estimatedSubmissionsRemaining).toBe(0);
  });

  it("returns null estimate when PLATFORM_FEE_STROOPS is not set", async () => {
    delete process.env.PLATFORM_FEE_STROOPS;
    const campaign = await createCampaign({ rewardStroops: 500000n });
    await createCampaignBalance(campaign.id, 4000000n);
